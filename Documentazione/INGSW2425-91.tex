...
\begin{titlepage}
    ...
    \begin{minipage}{0.45\textwidth}
        \raggedright
        \textbf{Studenti:}\\
        Pietro Orefice (N86004948)\\
        Gino Pandozzi-Trani (N86004852)
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\textwidth}
        \raggedleft
...
% === FRONT MATTER ===
\newpage
\section*{Lista di Distribuzione}
\addcontentsline{toc}{section}{Lista di Distribuzione}
\begin{table}[h]
\centering
\begin{tabularx}{\textwidth}{|l|l|c|}
\hline
\rowcolor[HTML]{010066} 
{\color[HTML]{FFFFFF} \textbf{Nome}} & {\color[HTML]{FFFFFF} \textbf{Ruolo}} & {\color[HTML]{FFFFFF} \textbf{Contatto}} \\ \hline
Pietro Orefice & Studente & pietoro.orefice@unina.it \\ \hline
Gino Pandozzi-Trani & Studente & gino.pandozzi@unina.it \\ \hline
Prof. Sergio DI MARTINO & Docente & s.dimartino@unina.it \\ \hline
Prof. Luigi Libero Lucio STARACE & Docente & l.starace@unina.it \\ \hline
\end{tabularx}
\end{table}

\newpage
\section*{Documenti di Riferimento}
\addcontentsline{toc}{section}{Documenti di Riferimento}
\begin{itemize}
\item Università degli Studi di Napoli Federico II -- Corso di Ingegneria del Software (A.A. 2024/2025)
\item Documento Progettuale DietiEstates25 -- Versione 1.0
\item Linee guida per la documentazione software -- INGSW Standards
\item Progettazione/Requisiti.md -- Specifica formale dei requisiti
\item Progettazione/Test-Doc.md -- Strategia di testing
\item dietiestates-backend/test/backend.test.ts -- Suite di test unità
\end{itemize}
...
Offerta                & Proposta formale di acquisto o affitto di un immobile a un determinato prezzo, inviata da un cliente registrato e gestita da un agente. \\ \hline
Controproposta         & Risposta a un'offerta ricevuta con condizioni modificate, implementata come offerta manuale da parte dell'agente. \\ \hline
Agenzia immobiliare    & Organizzazione registrata che impiega agenti immobiliari e gestisce un portafoglio di annunci. \\ \hline
JWT (JSON Web Token)   & Standard per la trasmissione sicura di informazioni tra client e server tramite token firmati e con scadenza. \\ \hline
OAuth                  & Protocollo di delega dell'autorizzazione; nel progetto consente l'accesso tramite account Google senza gestire direttamente le credenziali. \\ \hline
Geoapify               & Servizio API esterno per il recupero di punti di interesse (scuole, parchi, trasporti) nelle vicinanze di un indirizzo geografico. \\ \hline
API REST               & Interfaccia di comunicazione tra frontend e backend basata su HTTP; le risorse sono identificate tramite URL e i dati scambiati in formato JSON. \\ \hline
DTO (Data Transfer Object) & Oggetto intermediario usato per trasferire dati tra i livelli dell'applicazione; nel progetto validato con la libreria Zod. \\ \hline
DAO (Data Access Object)   & Strato di accesso al database che astrae le query SQL dal resto della logica applicativa, garantendo separazione delle responsabilità. \\ \hline
\end{tabularx}
\end{table}
